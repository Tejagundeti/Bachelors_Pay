  ...
  try {
    ...
    if (loan.lenderId !== session.user.id) {
      return NextResponse.json(
        { error: 'Only the requested lender can approve this loan' },
        { status: 403 }
      );
    }
    ...
    if (loan.status !== 'PENDING') {
      return NextResponse.json(
        { error: `Cannot approve a loan in ${loan.status} status` },
        { status: 400 }
      );
    }

    // Approve in transaction, notify borrower
    const updatedLoan = await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          type: 'LOAN_APPROVED',
          title: 'Loan Approved',
          message: `${loan.lender?.name || 'Someone'} approved your loan request of ₹${loan.amount.toFixed(2)}`,
          data: { loanId },
        },
      });
